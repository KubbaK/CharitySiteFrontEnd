...
const GetEventsVolunteering = () => {
  ...
  const fetchEvents = async (pageNumber) => {
    axios.get(`http://localhost:5012/v1/Search/pagination?isVerified=true&isVolunteering=true&sortBy=CreatedEventDate&sortDirection=DESC&pageNumber=${pageNumber}&pageSize=${eventsPerPage}`,{headers:{Authorization: `Bearer ${token}`}})
    .then(response => {
